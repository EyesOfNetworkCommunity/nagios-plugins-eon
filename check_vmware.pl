...
#         0.12 : More detail in disk space report, fix perfinfo for disks
#         0.13 : Network stats added
#         1.13 : Renumbered because version number bug fixed
#
...
my($WARNNET,$CRITNET)     = (7680,10240); # Kbps max
my($WARNFAIR,$CRITFAIR)   = (90,80); # percent min
my($WARNACTIVE,$CRITACTIVE)=(90,95); # percent max
...
my($MAXNSCA) = 10;
my($MAXGUESTCPUS) = 16; # guests cant have more than this many CPUs
my($NEWLINE) = "\\n"; # use \\n for nag3, <BR> for nag2
##########################################################################
##########################################################################
$Util::script_version = $VERSION; # doesnt work in current toolkit
...
sub getcounters($) {
	...
	foreach ( @$perfCounterInfo ) {
		print "  ".$_->groupInfo->key.":".$_->nameInfo->key."\n" if($DEBUG>2);
...
#				} else {
			...
			if( !$cpufair ) {	
# CPU fairness only available at HOST level, not Cluster
...
#			begin_entity => @$begin);
	...
	if($isnagios) {
		...
		if(defined $memavg) {	
			...
			$PERF.="mem=0\%;$WARNMEM;$CRITMEM;0;100 ";
		}
		if(!defined $memavg) {
			$STATUS = 3;
...
#				}
			...
			if( !$memfair ) {	
# Memory fairness data only available at host level
		...
		if($isnagios) {
			...
			if($freespace < $CRITSPACE*1024000000) {
				...
				$MSG .= "[C] ".$ds->info->name.": ".int($freespace/1024000)
					."MB ".($totspace?
					("(".(int($freespace/$totspace*1000)/10)."\%) "):"")."free";
				...
				$MSG = $ds->info->name.": ".int($freespace/1024000)
					."MB ".($totspace?
					("(".(int($freespace/$totspace*1000)/10)."\%) "):"")."free";
			}
			$PERF .= $ds->info->name."-free=".$freespace."B;$WARNSPACE;$CRITSPACE;0;$totspace "
				.$ds->info->name."=".($totspace-$freespace)."B;".($totspace-$WARNSPACE).";".($totspace-$CRITSPACE).";0;$totspace ";
		} else {
			$totspace += $ds->summary->capacity;
			$freespace += $ds->summary->freeSpace;
...
#			begin_entity => @$begin);
	...
	# we need to identify which is the Network usage counter.
	getcounters('net');
	...
	# Do any of these have instances??
	foreach ( qw/net:usage net:received net:transmitted/ ) {
		push @metricids, PerfMetricId->new (counterId => $perfkeys{$_},
			instance => '' ) if(defined $perfkeys{$_});
	}
	foreach ( @metricids ) {
	...
	# or a guest.
	if($isnagios) {
		my($netavg) = $results{$perfkeys{'net:usage'}};
		if(defined $netavg) {
			$PERF .= "net=".($netavg*8).";$WARNNET;$CRITNET;0; ";
		} else {
			$PERF .= "net=;$WARNNET;$CRITNET;0; ";
		}
		if(!defined $netavg) {
			$STATUS = 3;
			$MSG = "Network usage is unknown?";
		} elsif($netavg > $CRITNET) {
			$STATUS = 2;
			$MSG = "CRIT: Network usage at ".(int($netavg*100*8)/100)."Kbps (need more resource shares or physical interfaces?)";
		} elsif($netavg > $WARNNET) {
			$STATUS = 1;
			$MSG = "WARN: Network usage at ".(int($netavg*100*8)/100)."Kbps";
		} else {
			$STATUS = 0;
			$MSG = "Network usage at ".(int($netavg*100*8)/100)."Kbps";
		}
	} else {
		$A = $results{$perfkeys{'net:usage'}};
		if(!defined $A) { 
			$MSG = "Avg Network usage unknown";
			$A = "UNKNOWN"; 
		} else {
			$MSG = "Avg Network usage: ".(int($A*100*8)/100)." Kbps";
			$A *= 8 * 1024; # convert to bps
		}
		$B = $A;
		if($mode == 1) {
			$B = $results{$perfkeys{'net:received'}}; # recvd by ESX, ie out of vm
			$A = $results{$perfkeys{'net:transmitted'}};
			$A *= 8 * 1024 if($A);
			$B *= 8 * 1024 if($B);
			$A = "UNKNOWN" if(!defined $A);
			$B = "UNKNOWN" if(!defined $B);
			$MSG = "Sent/Recvd: ".(int($A/10.24)/100)."/".(int($B/10.24)/100)." Kbps";
		}
	}
}
...
if( Opts::option_is_set('warn') ) {
	if($report =~ /cpu/) {
		...
		$WARNNET = Opts::get_option('warn');
		if($WARNNET<1 or $WARNNET>99) { 
			print "Usage: 0<warn Kbps\n"; exit 3; }
	}
}
if( Opts::option_is_set('crit') ) {
	if($report =~ /cpu/) {
		$CRITCPU = Opts::get_option('crit');
		if($CRITCPU<$WARNCPU or $CRITCPU>100) { 
			print "Usage: warn<crit<100\%\n"; exit 3; }
	} elsif($report =~ /mem/ ) {
		$CRITMEM = Opts::get_option('crit');
		if($CRITMEM<$WARNMEM or $CRITMEM>100) { 
			print "Usage: warn<crit<100\%\n"; exit 3; }
	} elsif($report =~ /disk|data/ ) {
		...
		$CRITNET = Opts::get_option('crit');
		if($CRITNET<$WARNNET or $CRITNET>100) { 
			print "Usage: warn<crit Kbps\n"; exit 3; }
	}
}
...
#	if( (time-$s[9])>1200 ) {
#		# session file is >20mins old, lets reconnect		unlink $SESSIONFILE;
#		unlink $SESSIONFILE;
