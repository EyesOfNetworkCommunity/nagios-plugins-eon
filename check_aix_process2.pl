...
my $win_serv_name = '1.3.6.1.4.1.2.6.191.7.2.1.6';
my $win_serv_state = '1.3.6.1.4.1.2.6.191.7.2.1.9';

# Windows SNMP DATA
my %win_serv_state_label = ( 1 => 'active', 2=> 'runnable', 3=> 'notrunnable', 4=> 'invalid');
...
if (defined ($o_version2)) {
	$resultat = (Net::SNMP->VERSION lt 4) ? 
		...
		: $session->get_table(Baseoid => $win_serv_name,maxrepetitions => 10);
} else {
        $resultat = (Net::SNMP->VERSION lt 4) ?
                $session->get_table($win_serv_name)
...
if ( $num_int == 0) {
   if (defined ($o_number) && ($o_number ==0)) {
    print "No process ",(defined ($o_noreg)) ? "named \"" : "matching \"", $o_descr, "\" found : OK\n";
    exit $ERRORS{"OK"};
  } else  {
...
if (defined ($output) ) {
  ...
  print $num_int_ok, " process active (", (defined ($o_noreg)) ? "named \"" : "matching \"", $o_descr, "\") : ";
}
