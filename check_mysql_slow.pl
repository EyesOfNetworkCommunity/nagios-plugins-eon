#!/usr/bin/perl -w
#
# check_mysql_slow.pl - nagios plugin
#
#
...
my $PROGNAME = "check_mysql_slow";
my $VERSION  = "v1.1";

my $tmp = "/tmp/mysql_slow";
...
my $qps = $delta{'Slow_queries'} / $delta{'Uptime'};
my $tps = $delta{'Slow_launch_threads'} / $delta{'Uptime'};

$state = "OK";
if ($qps >= $opt_w || $tps >= $opt_w) { $state = "WARNING"; }
if ($qps >= $opt_c || $tps >= $opt_c) { $state = "CRITICAL"; }

my $perfdata = sprintf("slow_query=%.1f slow_thread=%.1f", $qps, $tps);

print sprintf("$state: %.1f/s Slow Querys  %.1f/s Slow Thread.|$perfdata\n", $qps, $tps);
exit $ERRORS{$state};

sub usage (){
        printf "\nMissing arguments!\n";
        printf "\n";
        printf "check_mysql_slow -H <HOSTNAME>\n";
...
sub print_help (){
	...
	printf "check and check slow querys/threads.\n";
	printf "\nUsage:\n";
