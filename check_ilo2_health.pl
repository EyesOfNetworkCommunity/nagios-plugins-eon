...
# 1.58b   Wed, 23 Apr 2014 10:36:34 +0200
#   specify "file:example.txt" as password to read the password from the first
#     line of that file
...
if ( substr($password, 0, 5) eq 'file:' ) {
    my $pwdfile = substr $password, 5;
    open(PWDFILE, $pwdfile) or $p->nagios_die("ERROR: $pwdfile not found");
    $password = <PWDFILE>;
    $password =~ s/\s+$//;   # strip newline from password
}
