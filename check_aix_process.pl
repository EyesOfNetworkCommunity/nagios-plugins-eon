...
# SNMP Datas
my $run_name_table = '1.3.6.1.4.1.2.6.191.7.2.1.6';
my $run_path_table = '1.3.6.1.4.1.2.6.191.7.2.1.6';
my $run_param_table = '1.3.6.1.4.1.2.6.191.7.2.1.6';
my $proc_mem_table = '1.3.6.1.2.1.25.5.1.1.2'; # Kbytes
my $proc_cpu_table = '1.3.6.1.4.1.2.6.191.7.2.1.7'; # Centi sec of CPU
my $proc_run_state = '1.3.6.1.4.1.2.6.191.7.2.1.9';
...
sub check_options {
    ...
    #### octet length checks
    if (defined ($o_octetlength) && (isnotnum($o_octetlength) || $o_octetlength > 65535 || $o_octetlength lt 484 )) {
...
if ( !defined ($o_path) ) {
  $resultat = (Net::SNMP->VERSION lt 4) ?
        ...
        :$session->get_table(Baseoid => $run_name_table,maxrepetitions => 10);
} else {
  $resultat = (Net::SNMP->VERSION lt 4) ?
	$session->get_table($run_path_table)
	:$session->get_table(Baseoid => $run_path_table,maxrepetitions => 10);
}
...
foreach my $key ( keys %$resultat) {
   ...
   $$resultat{$key}=decode_utf8($$resultat{$key});
   verb("OID : $key, Desc : ".decode_utf8($$resultat{$key})."");
